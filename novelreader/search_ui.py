...
class SearchMixin:
    ...
    def _open_search_dialog(self):
        ...
        dlg.title(_T("全文搜索 - {title}").format(title=self.book.title))
        ...
        def do_search(event=None):
            ...
            if not self._search_results:
                res_lbl.configure(text=_T("未找到“{q}”").format(q=q))
                return
            # 填充结果列表：章节号 + 上下文片段
            for k, (ci, i, _) in enumerate(self._search_results):
                try:
                    content = self.book.chapters[ci].content
                    title = self.book.chapters[ci].title.strip() or _T("第 {n} 章").format(n=ci + 1)
                    lo = max(0, i - 16)
                    ...
                    lb.insert("end", f"{k + 1}. " + _T("第 {n} 章").format(n=ci + 1))
            res_lbl.configure(text=_T("找到 {count} 处{extra}：双击结果跳转").format(count=count, extra=(_T("，仅显示前 500 条") if truncated else "")))
        ...
        def on_pick(event):
            ...
            self._flash_status(_T("已定位到“{q}”").format(q=q))
            # 可选：把关键词高亮一下
            try:
